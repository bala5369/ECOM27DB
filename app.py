...
import uuid
import razorpay
client = razorpay.Client(auth=("rzp_test_SzpueAQMszUU1s", "rHMhfNDJwnrBBbxQ1dAqRcXX"))
...
import os
def generate_reviews(price):
    if price > 30000:
        return [
            {
                "rating": 5,
                "review": "Premium quality product. Worth the price."
            },
            {
                "rating": 4,
                "review": "Excellent build quality and performance."
            }
        ]
    elif price > 15000:
        return [
        ...
        return [
            {
                "rating": 4,
                "review": "Budget-friendly and useful."
            },
            {
                "rating": 3,
...
def pay_cart():
    ...
    try:
        ...
        return jsonify({'status':'success','order':{
            ...
            'razorpay_key':'rzp_test_SzpueAQMszUU1s'})
    except Exception as e:
...
def verify_payment():
    ...
    try:
        ...
        grand_total=subtotal+tax+delivery
        #-------- STORE orders table
        cursor.execute('''insert into orders(razorpay_ordid,razorpay_payment,userid,total_amount,delivery,tax,grand_total,order_status) values(%s,%s,uuid_to_bin(%s),%s,%s,%s,%s,%s)''',[order_id,payment_id,userid,subtotal,delivery,tax,grand_total,'paid'])
...
def myorders():
    ...
    try:
        ...
        userid=session.get('userid')
        #fetch all orders
        cursor.execute('select orderid,razorpay_ordid,razorpay_payment,total_amount,delivery,tax,grand_total,order_status,created_at from orders where userid=uuid_to_bin(%s) order by created_at desc',[userid])
...
def usersearch():
    ...
    try:
        ...
        mydb.ping(reconnect=True)
        cursor = mydb.cursor()
        cursor.execute(""" SELECT BIN_TO_UUID(itemid),item_name,item_description,item_about,price,quantity,category,item_filename FROM items WHERE item_name LIKE %s """,(f'%{searchdata}%',))
        allitems_data = cursor.fetchall()
        items = []
        for item in allitems_data:
            items.append({
                ...
                'category': item[6],
                'image': url_for('static',filename=f'uploads/{item[7]}',_external=True)})
        return jsonify({'status': 'success','products': items}), 200
    except Exception as e:
        print('Mysql error:', str(e))
        return jsonify({'status': 'failed','message': str(e)}), 500
    finally:
...
def view_order(orderid):
    ...
    try:
        ...
        cursor.execute("""
            SELECT
                orderid,razorpay_ordid,razorpay_payment,total_amount,delivery,tax,grand_total,created_at
            FROM orders WHERE orderid=%s AND userid=uuid_to_bin(%s) """, [orderid, userid])
        ...
        # Fetch ordered items
        cursor.execute("""
            SELECT bin_to_uuid(itemid),item_name,item_price,item_quantity,subtotal,item_category,item_filename FROM order_items WHERE orderid=%s """, [orderid])
        items = cursor.fetchall()
        print("ORDER ITEMS:", items)  # DEBUG
        order_items = []
        for item in items:
            filename = item[6]
            print("IMAGE FILE:", filename)  # DEBUG
            image_url = None
            if filename:
                image_url = url_for(
                    'static',
        ...
        print("ORDER RESPONSE:", response)  # DEBUG
        return jsonify(response), 200
    except Exception as e:
        print("VIEW ORDER ERROR:", str(e))
        return jsonify({'status': 'failed','message': str(e)}), 500
    finally:
        if cursor:
            cursor.close()
...
def buy_now():
    ...
    try:
        ...
        # Store single item purchase in session
        session['single_buy'] = {
            'itemid': item_data[0],
            'itemname': item_data[1],
        ...
        print("SINGLE BUY SESSION:", session['single_buy'])

        return jsonify({
            'status': 'success',
            'message': 'Single buy item stored successfully',
            'payment_type': 'single',
            'item': session['single_buy'],
...
@app.route('/api/orders/<int:orderid>/invoice', methods=['GET'])
def download_invoice(orderid):
    cursor = None
    try:
        if 'userid' not in session:
            return jsonify({'status': 'failed','message': 'Please login first'}), 401
...
""", [orderid, userid])
        ...
        status = order[6] if order[6] else "Paid"
        elements.append(
            Paragraph(
                f"Order Status: {order[6]}",
                styles['Normal']
            )
        )
        ...
        elements.append(
            Paragraph(
                f"<b>Grand Total: ₹{float(order[4])}</b>",
                styles['Heading2']
            )
        )
        ...
        if order[6] == "Cancelled":
            ...
            elements.append(
                Paragraph(
                    "This order has been cancelled. Refund will be credited to the original payment method.",
                    styles['Normal']
                )
            )
        ...
        return send_file(
            buffer,
            as_attachment=True,
            download_name=f"invoice_{orderid}.pdf",
...
@app.route('/api/orders/<int:orderid>/cancel', methods=['PUT'])
...
@app.route('/api/orders/<int:orderid>/refund', methods=['PUT'])
...
@app.route('/api/orders/<int:orderid>/return', methods=['PUT'])
...
@app.route('/api/orders/<int:orderid>/track', methods=['GET'])
...
@app.route('/api/admin/orders/<int:orderid>/status', methods=['PUT'])
...
@app.route('/api/product/<itemid>', methods=['GET'])
...
@app.route('/api/product/<string:itemid>/reviews', methods=['GET'])
def get_reviews(itemid):

    mydb = get_db()          # NEW CONNECTION EVERY REQUEST
    cursor = None
    ...
    try:
        ...
        for row in rows:
            ...
            if row[2]:
                review_date = row[2].strftime('%Y-%m-%d %H:%M:%S')
        ...
        # Generate default reviews if no reviews exist
        if not reviews:
            ...
            if item:
                reviews = generate_reviews(float(item[0]))
...
@app.route('/api/product/<string:itemid>/review', methods=['POST'])
def add_review(itemid):
    ...
    try:
        ...
        if not userid or not rating or not review:
            return jsonify({
                'status': 'failed',
                'message': 'userid, rating and review are required'
            }), 400
